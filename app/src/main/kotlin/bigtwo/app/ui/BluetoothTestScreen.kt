...
fun BluetoothTestScreen(bluetoothPermissionManager: BluetoothPermissionManager) {
        ...
        if (!isDiscoverable && isServerStarted && bluetoothAdapter?.scanMode != BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE) {
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && !hasBluetoothConnectPermission) {
                    Log.w(
                        "BluetoothTestScreen",
                        "BLUETOOTH_ADVERTISE (implied) 权限未授予，无法请求可发现性。"
                    )
                } else {
                ...
                Toast.makeText(context, "无法请求蓝牙可发现性，请手动打开。", Toast.LENGTH_LONG)
                    .show()
            }
        }
    }
    ...
    LaunchedEffect(client) {
        client.messageReceived.collect { receivedMessage ->
            ...
            Log.d(
                "UI_UPDATE_CLIENT",
                "客户端UI已添加消息: $cleanedMessage, 当前列表大小: ${receivedMessages.size}"
            )
        }
    }
        ...
        Column(
                                ...
                                server.startServer(
                                    onMessageReceived = { received, senderId ->
                                        coroutineScope.launch(Dispatchers.Main) {
                                            ...
                                            val cleanedReceived =
                                                received.trimEnd() // !!! 新增：移除末尾的换行符或空格 !!!
                                            receivedMessages.add("收到来自 $clientName: $cleanedReceived") // 使用清理后的消息
                                            Log.d(
                                                "UI_UPDATE_SERVER",
                                                "服务端UI已添加消息: 收到来自 $clientName: $cleanedReceived, 当前列表大小: ${receivedMessages.size}"
                                            )
                                        }
                                    },
                                    onClientConnected = { clientId, clientName ->
                                        coroutineScope.launch(Dispatchers.Main) {
                                            ...
                                            Log.d(
                                                "BluetoothTestScreen",
                                                "服务端收到客户端 $clientName 连接通知 (UI)"
                                            )
                                        }
                                    },
                                    onClientDisconnected = { disconnectedClientId ->
                                        coroutineScope.launch(Dispatchers.Main) {
                                            val disconnectedClientName =
                                                connectedClients.remove(disconnectedClientId)
                                            Toast.makeText(
                                                context,
                                                "客户端 ${disconnectedClientName ?: disconnectedClientId} 已断开",
                                                Toast.LENGTH_SHORT
                                            ).show()
                                            Log.d(
                                                "BluetoothTestScreen",
                                                "服务端收到客户端 ${disconnectedClientName ?: disconnectedClientId} 断开连接通知 (UI)"
                                            )
                                        }
                                    }
                                )
            ...
            Button(
                onClick = {
                    ...
                    val requiredScanPermissionGranted =
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    ...
                    if (requiredScanPermissionGranted && isBluetoothEnabled && isLocationServiceEnabled) {
                        ...
                        coroutineScope.launch(Dispatchers.IO) {
                            ...
                            withContext(Dispatchers.Main) {
                                Toast.makeText(context, "开始扫描蓝牙设备...", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        }
                    } else {
                        ...
                        if (!isBluetoothEnabled) {
                            ...
                            // 检查是否是被“永久拒绝”了某个扫描权限
                            val permissionsToCheckForPermanentDenial =
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                bluetoothAdapter?.bondedDevices?.let { pairedDevices ->
                    items(pairedDevices.toList()) { device ->
                        PairedDeviceItem(device = device, onDeviceSelected = { selectedDevice ->
                            ...
                            Toast.makeText(
                                context,
                                "选中设备: ${selectedDevice.name ?: selectedDevice.address}",
                                Toast.LENGTH_SHORT
                            ).show()
                        })
                    }
...
//                                    },
            ...
            Text("接收到的消息:")
            val listState =
                rememberLazyListState() // 导入 androidx.compose.foundation.lazy.rememberLazyListState
